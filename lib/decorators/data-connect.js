  ...
  return function wrapWithDataConnect (WrappedComponent) {
    class ConnectData extends Component {
      ...
      constructor (props, context) {
        ...
        if (Object.keys(props.relateConnectorData).length === 0) {
          // Fetch data
          const hadDataToFetch = initialBundle && this.hasDataToFetch(initialBundle.fragments) && true;
          if (hadDataToFetch) {
            this.fetchData({
      ...
      () => function map (state, props) {
        if (!this.CONNECTOR_ID) {
          if (state.relateReducer.server) {
            const initialBundle = getBundle(props);
            const thisCompare = {
              fragments: initialBundle.fragments,
              variables: getVariables({
                variables: initialBundle.initialVariables,
